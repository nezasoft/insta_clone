        ...
        {data.map((item)=>(
            <div className="home" key={item._id}>
                <Card className={classes.root}>
                    <CardHeader
                    avatar ={<Avatar>
                        <img className={classes.avatar} alt="" src={`data: ${item.PhotoType};base64,${item.Photo}`} />

                    </Avatar>
                    }
                    title={
                        <Link className={classes.links} to={item.PostedBy._id !== state._id ? `/profile/${item.PostedBy._id}` : "/profile" }>
                            {item.PostedBy.Name}
                        </Link>
                    }
                    subheader ="29th May 2023"
                 />
            <CardMedia className={classes.media} image={`data:${item.PhotoType};base64,${item.Photo}`} title="Paella dish"/>    
            <CardActions className={classes.likeBar} disableSpacing>
                {item.Likes.includes(state._id) ? (
                    <IconButton label="Like" color="secondary" onClick={()=>{unlikePost(item._id)}} >
                        <FavoriteIcon />
                    </IconButton>
                ...
                {state.Bookmarks.includes(item._id) ? (
                    <IconButton aria-label="Remove Bookmark" style={{marginLeft: "auto", color:"#e0d011"}} onClick={()=>{removeBookmark(item._id);}}>
                        <BookmarkIcon />
                    </IconButton>
            ...
            <List>
                {item.Comments.map((cmt) => {
                    return (
                        <ListItem className={classes.comment_item} alignItems="flex-start" key={cmt.id}>
                            <ListItemText secondary={
                                <React.Fragment>
                                    <Typography component="span" variant="body2" className={classes.inline} color="textPrimary">
                                        <Link className={classes.links} to={cmt.PostedBy._id !== state._id ? `/profile/${cmt.PostedBy._id}` : "/profile"}>
                                            {cmt.PostedBy.Name}
                                        </Link>
                                    </Typography>
                                    {"-"}
            ...
            <CardContent className={classes.comments}>
                ...
                />
                <IconButton aria-label="add to favorites" className={classes.comments_icon} disabled={!showSend} onClick={()=>{makeComment(comment,item._id)}}>
                    <SendIcon/>
                </IconButton>
            </CardContent>
        </Card>
